...
from accounts.email_utils import send_registration_welcome
...
def network(request):
    ...
    for lvl in range(1, 21):
        ...
        if not members and lvl > 3:  # Stop after level 3 if no more members
            break
        level_stats.append({
            'level': lvl,
            'count': len(members),
...
def profile(request):
    ...
    if request.method == 'POST':
        # Update TRC20 withdrawal address
        trc20_address = request.POST.get('trc20_wallet_address', '').strip()
        if trc20_address:
            if len(trc20_address) >= 34 and trc20_address.startswith('T'):
                user.trc20_wallet_address = trc20_address
                user.save()
                messages.success(request, "Withdrawal address updated successfully.")
            else:
                messages.error(request, "Invalid TRC20 address format. Must start with 'T' and be at least 34 characters.")
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        # Log user in
        login(request, user)
        messages.success(request, f"Welcome to Phonix! You've received a $10 registration bonus.")
